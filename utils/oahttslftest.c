 ...
 * 
 * Usage:
 *    oahttslftest [-r] [numthreads]
 *
 *     -r : read actions from stdin
 *
 * $Id: oahttslftest.c 4556 2013-01-13 02:20:29Z astivala $
 *
 *****************************************************************************/
...
static void *insert_random(void *threadarg)
{
  ...
  for (q = 0; q < mydata->num_insertions; q++)
  {
    ...
    if (s.low == 0)
        s.low = 1;
    s.high = 0;
    if (!oahttslf_lookup(s.low, &value))
    {
      ...
      if (snew.low == 0)
          snew.low = 1;
      snew.high = 0;
      if (oahttslf_lookup(snew.low, &ivalue))
      {
        if (ivalue != snew.low) {
          fprintf(stderr, "ASSERTION FAILURE: thread %d: ivalue=%llX snew.low=%llX\n",  mydata->thread_id, ivalue, snew.low);
            exit(100);
        }
      }

      value = (uint64_t)s.low;
      oahttslf_insert(s.low, value, mydata->thread_id);
    }
    else
    {
    /*  assert(value == (uint64_t)s.low); */
      if (value != s.low) {
          fprintf(stderr, "ASSERTION FAILURE 2: thread %d: value=%llX s.low=%llX\n",  mydata->thread_id, value, s.low);
          exit(101);
      }
    }
  }
