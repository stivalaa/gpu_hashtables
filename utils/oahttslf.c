 ...
 *
 * $Id: oahttslf.c 4497 2012-12-31 05:58:43Z astivala $
 *
 *
...
static unsigned long long collision_count[MAX_NUM_THREADS];
#endif
...
uint64_t oahttslf_insert(uint64_t key, uint64_t value, int thread_id)
{
  ...
  ent = oahttslf_getent(key, &vacant, thread_id);
  if (!ent)
  {
...
bool oahttslf_lookup(uint64_t key, uint64_t *value)
{
  ...
  ent = oahttslf_getent(key, &vacant, -1);
  if (ent)
  {
    val = ent->value;
 ...
 *    thread_id - our thread identifer (0,1,2,.. NOT pthread_t) used
 *                 only for instrumentation. If <0 do not count 
 *                probes (so used only for collision counts on insert)
 *  Return value:
                                           ...
                                           int thread_id)
{
  ...
  if (thread_id >= 0)
  {
    collision_count[thread_id] += probes;
  }
#endif
...
/*
 *  add up the per-thread collision counters and return total
 * Parameters: None
 * Return value: Total number of keys in the hash table
 */
unsigned long long oahttslf_total_collision_count()
{
  ...
  int i;
  for (i = 0; i < MAX_NUM_THREADS; i++)
    num_collisions += collision_count[i];
...
{
  static const char *funcname = "oahttslf_delete";
  volatile oahttslf_entry_t *ent;
  uint64_t entkey, oldvalue;
  bool vacant;
  ...
  {
    oldvalue = ent->value;
    if (CAS64(&ent->key, entkey, OAHTTSLF_DELETED_KEY) != entkey) {
#ifdef USE_CONTENTION_INSTRUMENT
      retry_count[thread_id]++;
#endif
      return oahttslf_delete(key, thread_id);  /* tail-recursive call to retry */
    }
#ifdef USE_INSTRUMENT
    key_count[thread_id]--;
#endif
