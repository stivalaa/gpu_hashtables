/**********************************************************************************
 ...
 This version changed by ADS to use the httslf hash table for comparison
 ...
 NB DELETE is unsupported on httslf
 ...
 KEYS (now a command line parameters)
 ...
 $Id: iit_httslf_test.cpp 4497 2012-12-31 05:58:43Z astivala $
***************************************************************************************/
...
//    //START
//    // simple test in single thread
//    uint64_t val = 0;
//    httslf_insert(123, 124);
//    bool foundit = httslf_lookup(123,  &val);
//    httslf_delete(987);
//    assert(foundit);
//    assert(val == 124);
//    httslf_delete(123);
//    foundit = httslf_lookup(123,  &val);
//    assert(!foundit);
//    //END
...
//         fprintf(stderr, "XXX ADD %lld, %lld\n", item, value);
         result[i]=10+httslf_insert(item, value, tid)->value;
         break;
       case DELETE:
#ifdef ALLOW_DELETE
         httslf_delete(item);
#endif
         result[i]=20; // don't know why the original does this int+bool

         break;
       case SEARCH:
//         fprintf(stderr, "XXX SERACH %lld\n", item);
         ++searchedcount;
         bool found = httslf_lookup(item, &value);
...
int main(int argc, char** argv)
{
  ...
  if (argc != 5) {
     printf("Need four arguments: number of keys and percent add ops and percent delete ops and number of threads (e.g., 100000 30 10 8 for 100000 keys with 30%% add and 10%% delete with 8 threads).\nAborting...\n");
     exit(1);
  }
