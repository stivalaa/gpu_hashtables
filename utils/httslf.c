 ...
 *
 * $Id: httslf.c 4457 2012-12-16 04:37:48Z astivala $
 *
 * Preprocessor symbols:
 *
 * USE_CP_ALLOC         - use cellpool allocator
 ...
 *    thread_id  - id (0..num_threads NOT pthread_t) of this thread
 *
 * Return value:
    ...
    if (!ent)
    {
      ...
      */
      if (!newent)
      {
        ...
        newcell = perthread_cellpool_alloc(thread_id);
        if (!newcell)
          bpa_fatal_error(funcname, "cellpool_alloc() failed\n");
        newent = (httslf_entry_t *)newcell;
#elif defined(USE_CP_ALLOC)
                   ...
                   copy_function_t valuecopy, int num_threads)
{
  ...
  cell_size = sizeof(httslf_entry_t) + key_size + value_size;
  /* need to pass number of cells as a parameter in this function */
  if (!(cellpool_initialize(cell_size, (size_t)10000000)))
      bpa_fatal_error(funcname, "cellpool_initialize() failed\n");
#endif
}
