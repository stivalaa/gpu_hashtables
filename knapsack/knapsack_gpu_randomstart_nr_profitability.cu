/*****************************************************************************
 *
 * File:    knapsack_gpu_randomstart_nr_profitability.cu
 * Author:  Alex Stivala & Peter J Stuckey
 * Created: April 2009
 *
 * $Id: knapsack_gpu_randomstart_nr_profitability.cu 4629 2013-01-25 04:28:40Z astivala $
 *
 ...
 *
 *
 *  Usage: knapsack_gpu_randomstart_nr_profitability [-nvy] [-r threads]  < problemspec
 *          -v: Verbose output 
...
// average weight of item
__constant__ float c_AVERAGE_WEIGHT;
const char *c_AVERAGE_WEIGHT_symbol = "c_AVERAGE_WEIGHT";
                                   ...
                                   HASHTABLE_TYPE devHashtable)
{
 ...
 unsigned int profit = 0;
  ...
  if (randomstart)
  {
     ...
     * number of threads..
     * This version instead of completely random uses PJS's idea of
     * only picking random in (1..n,  i*avgweight .. W)
     */  
    while (!finished)
    {
      curandState localState = state[tid];
      unsigned int random_i = curand(&localState) % i; 
      unsigned int lowerweight = (unsigned int)(random_i*c_AVERAGE_WEIGHT);
      unsigned int random_w = min(w, (curand(&localState) % (w - lowerweight)) + lowerweight);
      //DEBUG_PRINT(("tid = %d random_i = %u random_w = %u\n", tid, random_i, random_w));
      //assert(random_w <= w);
      //assert(random_w >= 0);
      state[tid] = localState;
...
#define DO_SORT_ITEMS
  ...
  for (i = 1; i <= NUM_KNAPSACK_ITEMS; i++) {
    bi = i;
    ri = (float)KNAPSACK_ITEMS[i].profit/(float)KNAPSACK_ITEMS[i].weight;
    for (j = i+1; j <= NUM_KNAPSACK_ITEMS; j++) {
      rj = (float)KNAPSACK_ITEMS[j].profit/(float)KNAPSACK_ITEMS[j].weight;
      if (rj > ri) {
	ri = rj;
	bi = j;
      }
    }
    w = KNAPSACK_ITEMS[i].weight; KNAPSACK_ITEMS[i].weight = KNAPSACK_ITEMS[bi].weight; KNAPSACK_ITEMS[bi].weight = w;
    w = KNAPSACK_ITEMS[i].profit; KNAPSACK_ITEMS[i].profit = KNAPSACK_ITEMS[bi].profit; KNAPSACK_ITEMS[bi].profit = w;
  }
  ...
  float average_weight = (float)total_weight / (float)NUM_KNAPSACK_ITEMS;

  DEBUG_PRINT(("num items = %u capacity = %u average weight = %f\n", NUM_KNAPSACK_ITEMS, CAPACITY, average_weight));
  ...
  /* copy average weight to device constant memory */
  if ((rc = cudaMemcpyToSymbol(c_AVERAGE_WEIGHT_symbol,  &average_weight,
                               sizeof(average_weight))) != cudaSuccess) 
  {
    fprintf(stderr, "cudaMemcpyToSybmol AVERAGE_WEIGHT failed %d\n", rc);
    exit(1);
  }
