/*****************************************************************************
 *
 * File:    knapsack_gpu_nr_oahttslf.cu
 * Author:  Alex Stivala & Peter J Stuckey
 * Created: April 2009
 *
 * $Id: knapsack_gpu_nr_oahttslf.cu 4635 2013-01-31 03:24:47Z astivala $
 *
 * This is the GPU implementation using the oahttslf GPU lock free hash table.
 * This version does not use recursion.
 *
 *  Usage: knapsack_gpu_nr_oahttslf [-nvy] [-r threads]  < problemspec
 *          -v: Verbose output 
 ...
 * It uses the either th CURAND or Random123 library for 
 * pseudrandom number generation. CURAND is in the CUDA SDK.
 * Random123 is from  http://www.deshawresearch.com/resources_random123.html
 *
 * Preprocessor symbols:
 *
 * USE_RANDOM123  - use Random123 library instead of CURAND
 ...
 *                    w - total weight
 *                state/seed - CURAND state for random number generation /or
  *                            seed for Random123 
...
#ifdef USE_RANDOM123
  philox2x32_key_t r123_k = {{tid + seed}};
  philox2x32_ctr_t r123_c = {{0, 0xf00dcafe}};
...
#ifdef USE_RANDOM123
      philox2x32_ctr_t r123_ctr = philox2x32(r123_c, r123_k);
      randint = r123_ctr.v[0];
#else
      randint = curand(&localState);
#endif
      if (randint & 1)  {
    	top++;
    	stacki[STACKINDEX(top)] = i;
 ...
 *        state/seed - CURAND state for random number generation /or
 *                     seed for Random123 initial key
 * devHashtable - the hash table
 *
 *     global memory:
...
int main(int argc, char *argv[])
{
  ...
  dp_knapsack_kernel<<<dimGrid, dimBlock>>>(NUM_KNAPSACK_ITEMS, CAPACITY, d_profit,
#ifdef USE_RANDOM123
  time(NULL), // seed for init random123 key
#else
  devStates,
#endif
  devHashtable);
