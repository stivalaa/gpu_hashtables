...
###############################################################################
#
# runall_nonrecursive.sh - Run run_instances.py with different numbers of cores
#
# File:    runall_nonrecursive.sh
# Author:  Alex Stivala
# Created: April 2009
#
#
...
#
# Usage:
#  runall_nonrecursive.sh  knapsack_options outdir max_threads 
#
...
#
# $Id: runall_nonrecursive.sh 4544 2013-01-11 01:06:07Z astivala $
# 
###############################################################################
...
while [ $numthreads -ge 1 ]; do
  ...
  while [ $iter -le $MAXITERATIONS ]; do
      /usr/bin/time $TIMEGUARD $TIMEOUT run_instances.py  knapsack_nr_oahttslf "-r${numthreads} ${knapsack_options}" instances > ${outdir}/instances_oahttslf_${numthreads}_${iter}.out 2> ${outdir}/instances_oahttslf_${numthreads}_${iter}.err
      iter=`expr $iter + 1`
  done
