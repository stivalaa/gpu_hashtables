...
#
# $Id: runall_gpu_norandomstart.sh 4549 2013-01-12 22:28:33Z astivala $
# 
###############################################################################
...
while [ $iter -le $MAXITERATIONS ]; do
    /usr/bin/time $TIMEGUARD $TIMEOUT run_instances.py  knapsack_gpu_nr_oahttslf "${knapsack_options}" instances > ${outdir}/instances_oahttslf_NA_${iter}.out 2> ${outdir}/instances_oahttslf_NA_${iter}.err
    iter=`expr $iter + 1`
done
