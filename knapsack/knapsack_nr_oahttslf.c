/*****************************************************************************
 *
 * File:    knapsack_nr_oahttslf.c
 * Author:  Alex Stivala & Peter J Stuckey
 * Created: April 2009
 *
 * $Id: knapsack_nr_oahttslf.c 4424 2012-11-28 22:50:38Z astivala $
 *
 * This is non-recursive implementation using pthreads and the oahttslf lockfree
 * hashtable.
 *
 *
 *  Usage: knapsack_nr_oahttslf [-ntvyz] [-r threads]  < problemspec
...
#define MAXITEMS 10000 /* FIXME, used for stack size */

unsigned int dp_knapsack_nr(unsigned int i, unsigned int w, int thread_id,
                         unsigned int *seed);
 ...
 *
 */
static unsigned int lookk(unsigned int i, unsigned int w) 
{
  unsigned int p = 0;
  oahttslf_lookup_indices(i, w, &p);
  return p;
}
...
/*
 * dp_knapsack_thread() - thread interface to dp_knapsack_nr()
 *
 *      This version is multi-threaded, sharing hashtable used to
 *      store computed values between the threads. This function
 *      is just the pthreads interface to the non-recursive dp_knapsack_nr()
...
void *dp_knapsack_thread(void *threadarg)
{
  ...
  mydata->profit = dp_knapsack_nr(mydata->i, mydata->w, mydata->thread_id, &seed);
...
unsigned int dp_knapsack_nr(unsigned int i, unsigned int w, int thread_id,
                         unsigned int *seed)
{
  static const char *funcname = "dp_knapsack_nr";

  unsigned int p;
  int c;
  int stackc[MAXITEMS+1];  /* call or answer */
  unsigned int stacki[MAXITEMS+1];
  unsigned int stackw[MAXITEMS+1];
  int top = 1;
  ...
  while(top) {
    ...
    if (c) {
      if (i == 1) { 
	p = (w >= ITEMS[i].weight ? ITEMS[i].profit : 0);
#ifdef DEBUG
        printf("knapsack_nr(%d,%d,%d) = %d\n",i,w,c,p);
#endif    	
        oahttslf_insert_indices(i, w, p, thread_id);
      }
      else if (lookk(i-1,w) > 0 && lookk(i-1,w - ITEMS[i].weight) > 0) {
	p = MAX(lookk(i-1,w), 
                (w >= ITEMS[i].weight ? lookk(i-1,w - ITEMS[i].weight) + ITEMS[i].profit : 0));
#ifdef DEBUG
        printf("knapsack_nr(%d,%d,%d) = %d\n",i,w,c,p);
#endif    	
        oahttslf_insert_indices(i, w, p, thread_id);
    ...
    if (!use_random || rand_r(seed) % 2) {
    	...
    	if (i >= 1 && w >= ITEMS[i].weight && lookk(i-1,w-ITEMS[i].weight) == 0) {
    	  top++;
    	  stacki[top] = i-1;
    	  stackw[top] = w - ITEMS[i].weight;
    	  stackc[top] = TRUE;
    	}
     }
    else {
  ...
  oahttslf_insert_indices(oi, ow, p, thread_id);
#ifdef DEBUG
  printf("knapsack_nr(%d,%d) = %d\n",i,w,p);
#endif 
  return p;
