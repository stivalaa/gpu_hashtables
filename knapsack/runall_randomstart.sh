...
#
# $Id: runall_randomstart.sh 4565 2013-01-16 04:10:44Z astivala $
# 
###############################################################################
...
while [ $numthreads -ge 1 ]; do
  ...
  while [ $iter -le $MAXITERATIONS ]; do
      /usr/bin/time $TIMEGUARD $TIMEOUT run_instances.py  knapsack_randomstart_oahttslf "-r${numthreads} ${knapsack_options}" instances > ${outdir}/instances_oahttslf_${numthreads}_${iter}.out 2> ${outdir}/instances_oahttslf_${numthreads}_${iter}.err
      iter=`expr $iter + 1`
  done
