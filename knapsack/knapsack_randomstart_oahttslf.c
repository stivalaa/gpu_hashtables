/*****************************************************************************
 *
 * File:    knapsack_randomstart_oahttslf.c
 * Author:  Alex Stivala
 * Created: January 2013
 *
 * $Id: knapsack_randomstart_oahttslf.c 4564 2013-01-16 03:47:01Z astivala $
 *
 * This is the pthreads implementation using the oahttslf lockfree
 * hashtable. This version has an additional feature in which we try
 * stating some threads at random (i,w) positions in the problem rather
 * than starting all at the top (solution) values of (i, w)
 *
 *
 *  Usage: knapsack_randomstart_oahttslf [-ntvyz] [-r threads]  < problemspec
...
void *dp_knapsack_thread(void *threadarg)
{
   ...
   */
   if (mydata->thread_id % 2)
  //XXXif (mydata->thread_id > 0)
  {
     ...
     */
     int i = rand_r(&seed) % mydata->i;
     int w = rand_r(&seed) % mydata->w;
     while (TRUE)
     {
       (void)dp_knapsack(i, w, mydata->thread_id, &seed);
     }
  }
  else 
  {
    /* "normal" (original) case: start thread at the (i,w) value 
     * we actually want */
