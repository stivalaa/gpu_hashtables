...
#
# $Id: runall_gpu.sh 4544 2013-01-11 01:06:07Z astivala $
# 
###############################################################################
...
while [ $iter -le $MAXITERATIONS ]; do
    /usr/bin/time $TIMEGUARD $TIMEOUT run_instances.py  knapsack_gpu_randomstart_nr_oahttslf "${knapsack_options}" instances > ${outdir}/instances_oahttslf_NA_${iter}.out 2> ${outdir}/instances_oahttslf_NA_${iter}.err
    iter=`expr $iter + 1`
done
